...
const vButtons = (paused, speaking = false) => `<div class="vbuttons"><span class="big ${paused ? 'resume' : 'pause'}">${paused ? I.micf : I.pause}</span>${speaking ? `<span class="big shush" title="Quiet: cut Hermes off, the chat goes on">${I.shush}</span>` : ''}</div>`;
const V = [
  ...
  phone('2 · Hermes speaking <small>· the reply, with markdown; the speaker button cuts it off</small>', `${vTop('Hermes is speaking', 'speak')}
    <div class="vbody">
